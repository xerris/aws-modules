...
def discover_delete_images(regionname):
    ...
    for repository in repositories:
        if ((len(REPO_NAME) > 0) and ( repository['repositoryName'] == REPO_NAME )):
            EXEC_LOOP= True
...
if __name__ == '__main__':
    ...
    PARSER.add_argument('-imagestokeep', help='Number of image tags to keep', default='100', action='store',
                        dest='imagestokeep')
    PARSER.add_argument('-reponame', help='ECR repo name to clean', default='', action='store',
                        dest='reponame')
    ...
    os.environ["REPO_NAME"] = ARGS.reponame
